...
def make_svg(lines, out_path, static=False):
    ...
    for i, (k, v) in enumerate(lines):
      ...
      <text x="{key_indent}" y="{base_y + i * 120}" font-family="Inter, sans-serif" font-size="{label_font_size}" fill="#9aa3b2"><tspan font-weight="700">{safe_key}</tspan></text>
'''
            ...
            anim += '    </g>'
        if static:
            rendered = [f'<text x="{key_indent}" y="{base_y + i * 120}" font-family="Inter, sans-serif" font-size="{label_font_size}" fill="#9aa3b2"><tspan font-weight="700">{safe_key}</tspan></text>']
            for j, (line, y_pos) in enumerate(body_lines):
  ...
  <text x="18" y="28" font-family="Inter, sans-serif" font-size="{title_font_size}" fill="#c9d1d9">{escape(title)}</text>
  {"".join(items)}
</svg>'''
